...
public class CommandUtils
{
	...
	static
	{
		...
		try
		{
			...
			if (is118Minus)
			{
				...
				h.put(0, ArgumentTypeInfos.class.getMethod(mapped, String.class, Class.class, ArgumentTypeInfo.class));
			}
			
			mapped = mappingResolver.mapMethodName("intermediary", "net.minecraft.class_2096$class_2099", "method_9047", is116Minus ? "(F)Z" : "(D)Z");
			h.put(1, MinMaxBounds.Doubles.class.getMethod(mapped, is116Minus ? float.class : double.class));
			
			if (is119Minus)
			{
				mapped = mappingResolver.mapMethodName("intermediary", "net.minecraft.class_2168", "method_9226", "(Lnet/minecraft/class_2561;Z)V");
				h.put(2, CommandSourceStack.class.getMethod(mapped, Component.class, boolean.class));
			}
		}
	...
	public static <T extends ArgumentType<?>> void registerConstantArgumentType(ResourceLocation id, Class<? extends T> argClass, Supplier<T> supplier)
	{
		if (REGISTER_ARGUMENT_TYPE != null)
		{
			try
			{
				REGISTER_ARGUMENT_TYPE.invoke(null, id.toString(), argClass, SingletonArgumentInfo.class.getConstructor(Supplier.class).newInstance(supplier));
			}
			catch (Throwable e)
			{
				Pehkui.LOGGER.catching(e);
