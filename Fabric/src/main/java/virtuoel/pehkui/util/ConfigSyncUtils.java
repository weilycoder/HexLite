...
public class ConfigSyncUtils
{
	...
	public static void syncConfigs(final ServerGamePacketListenerImpl networkHandler, final Collection<SyncableConfigEntry<?>> configEntries)
	{
		if (NETWORKING_API_LOADED)
		{
			...
			{
				networkHandler.send(createConfigSyncPacket(configEntries));
			}
		}
	}
	
	@SuppressWarnings({ "unchecked", "rawtypes" })
	public static ClientboundCustomPayloadPacket createConfigSyncPacket(final Collection<SyncableConfigEntry<?>> configEntries)
	{
		...
		return new ClientboundCustomPayloadPacket(Pehkui.CONFIG_SYNC_PACKET, buffer);
	}
