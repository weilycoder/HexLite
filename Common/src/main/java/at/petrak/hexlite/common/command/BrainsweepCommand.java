                ...
                if (target instanceof Mob mob) {
                    if (IXplatAbstractions.INSTANCE.isBrainswept(mob)) {
                        ctx.getSource().sendFailure(
                            Component.translatable("command.hexlite.brainsweep.fail.already",
                                mob.getDisplayName()));
                        return 0;
                    }
                    HexAPI.instance().brainsweep(mob);
                    ctx.getSource().sendSuccess(
                        () -> Component.translatable("command.hexlite.brainsweep", mob.getDisplayName()), true);
                    return 1;
                } else {
                    ctx.getSource().sendFailure(
                        Component.translatable("command.hexlite.brainsweep.fail.badtype",
