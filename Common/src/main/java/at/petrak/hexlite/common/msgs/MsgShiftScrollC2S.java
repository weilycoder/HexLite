package at.petrak.hexlite.common.msgs;

import at.petrak.hexlite.api.casting.iota.IotaType;
import at.petrak.hexlite.api.utils.NBTHelper;
import at.petrak.hexlite.common.items.storage.ItemAbacus;
import at.petrak.hexlite.common.items.storage.ItemSpellbook;
...
public record MsgShiftScrollC2S(double mainHandDelta, double offHandDelta, boolean isCtrl, boolean invertSpellbook,
    ...
    private void spellbook(ServerPlayer sender, InteractionHand hand, ItemStack stack, double delta) {
        ...
        if (hand == InteractionHand.OFF_HAND && stack.hasCustomHoverName()) {
            if (sealed) {
                component = Component.translatable("hexlite.tooltip.spellbook.page_with_name.sealed",
                    ...
                    Component.translatable("hexlite.tooltip.spellbook.sealed").withStyle(ChatFormatting.GOLD));
            } else {
                component = Component.translatable("hexlite.tooltip.spellbook.page_with_name",
                    Component.literal(String.valueOf(newIdx)).withStyle(ChatFormatting.WHITE),
                    Component.literal(String.valueOf(len)).withStyle(ChatFormatting.WHITE),
                    Component.literal("").withStyle(stack.getRarity().color, ChatFormatting.ITALIC)
                        .append(stack.getHoverName()));
            ...
            if (sealed) {
                component = Component.translatable("hexlite.tooltip.spellbook.page.sealed",
                    Component.literal(String.valueOf(newIdx)).withStyle(ChatFormatting.WHITE),
                    Component.literal(String.valueOf(len)).withStyle(ChatFormatting.WHITE),
                    Component.translatable("hexlite.tooltip.spellbook.sealed").withStyle(ChatFormatting.GOLD));
            } else {
                component = Component.translatable("hexlite.tooltip.spellbook.page",
                    Component.literal(String.valueOf(newIdx)).withStyle(ChatFormatting.WHITE),
                    Component.literal(String.valueOf(len)).withStyle(ChatFormatting.WHITE));
            }
        }
    ...
    private void abacus(ServerPlayer sender, InteractionHand hand, ItemStack stack, double delta) {
        ...
        if (datumTag != null) {
            ...
            sender.displayClientMessage(
                Component.translatable("hexlite.tooltip.abacus", popup).withStyle(ChatFormatting.GREEN), true);
        }
    }
}
